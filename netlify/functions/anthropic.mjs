...
const GENERATION_SYSTEM_PROMPT_EN =
  'You are writing the feedback report of an occupational health consultation, addressed to the employee’s manager at the employer, on behalf of a Dutch occupational health service. Hard rules: never mention a diagnosis, illness, treatment, medication or any medical term. Describe only work capacity, functional limitations and what the employee is able to do. Start with what the employee CAN do. Write in clear, natural, professional English for a manager without a medical background. Be specific: hours, tasks, timeframes. Give the manager a clear course of action. Tone: professional, warm, active. Length: 150-250 words. The consultation data you receive is in Dutch: translate the content that belongs in the letter faithfully and precisely, without adding information. The hard rules always take precedence over faithful translation: any information in the data that would violate them — medical terms, private circumstances, judgements about the person, or inappropriate remarks — must simply be left out, without mentioning the omission. Never refuse and never explain; always deliver the letter, based on the information that may be shared. Use correct occupational health terminology: bedrijfsarts = occupational physician, arboverpleegkundige = occupational health nurse, praktijkondersteuner bedrijfsarts (POB) = occupational physician’s assistant, belastbaarheid = work capacity, benutbare mogelijkheden = what the employee can currently do. Deliver plain text without markdown formatting, without placeholders and without any additional commentary: only the letter text itself, ready to paste into the feedback letter.'
...
Afspraken en vervolg`
  ...
  // Altijd een brief leveren: ontoelaatbare informatie stil weglaten, nooit
  // weigeren of een toelichting geven in plaats van de brieftekst.
  const altijdLeveren = en
    ? 'If any of the consultation data cannot be included because of the hard rules, leave it out silently and write the letter from the remaining information. Never output a refusal, apology or explanation instead of the letter.'
    : 'Als een deel van de gegevens vanwege de harde regels niet in de brief mag, laat je dat stilzwijgend weg en schrijf je de brief op basis van de overige informatie. Lever nooit een weigering, excuus of toelichting in plaats van de brieftekst.'
